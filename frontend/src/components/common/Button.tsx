...
const spinnerClass = 'h-4 w-4 animate-spin rounded-full border-2 border-white/40 border-t-white';

export const Button = forwardRef<HTMLButtonElement, ButtonProps>(function Button(
  { variant = 'primary', size = 'md', loading = false, startIcon, block = false, className = '', type = 'button', disabled, children, ...rest },
  ref
) {
  const contentMuted = loading ? 'text-transparent' : '';
  const mergedDisabled = disabled || loading;
  const widthClass = block ? 'w-full' : '';
  const composedClassName = `inline-flex items-center justify-center gap-1 rounded-md font-medium transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:cursor-not-allowed disabled:opacity-60 ${variantClasses[variant]} ${sizeClasses[size]} ${widthClass} ${className}`.trim();
